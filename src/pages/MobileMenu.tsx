...
const MobileMenu = () => {
  ...
  const confirmAddToCart = () => {
    ...
    const mId = (selectedItem.id || selectedItem._id).toString();
    setCart(prev => {
      const existing = prev.findIndex(c => c.menuItemId === mId && c.modifiers.sort().join(',') === modifiers.sort().join(',') && c.notes === notes);
      if (existing >= 0) {
        const next = [...prev];
        next[existing].qty += 1;
        return next;
